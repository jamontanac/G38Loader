...
def variant_record(row: pd.Series) -> dict[str, Any]:
    ...
    # Extract VariationID directly from the row
    variation_id = str(row["VariationID"])

    # RS ID logic (unchanged)
    if rs_raw in ("-1", "nan", ""):
        rs_id = "NA"
        variante = "NA"
    else:
    ...
    # NEW logic for ClinVar link
    # Use VariationID instead of RS ID
...
def main() -> None:
    ...
    parser.add_argument(
        "--output", type=Path, default=None, help="Override output JSON path"
    )
    ...
    input_file = args.input or db_path(
        cfg, "clinvar", cfg["clinvar"]["variant_summary_file"]
    )
